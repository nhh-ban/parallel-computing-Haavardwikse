...
library(tweedie)
library(ggplot2)
library(parallel) # Added for parallel computing
...
MTweedieTests <- function(N, M, sig) {
  numCores <- detectCores() # Number of cores to use for parallel processing
  
  # Split M simulations across the available cores
  results <- mclapply(1:numCores, function(core) {
    localM <- M / numCores # Split M equally across cores
    sum(replicate(localM, simTweedieTest(N)) < sig)
  }, mc.cores = numCores)
  
  # Aggregate results from all cores
  totalSum <- sum(unlist(results))
  return(totalSum / M)
}
